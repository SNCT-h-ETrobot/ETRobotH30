...
public class GamePartDriver {
	...
	private final float BLOCK_MOVE_DISTANCE = 6.0F;
	...
	public void driveGamePart()
	{
		...
		while(true)
		{
			...
			if(colorDetect.getUnderColorID() == 1)
			{
				break;
			}
				...
				if(distance >= 0.0f)
				{
					...
					*/
					...
					else
					{
						...
						public void run(){
							if(distMeasure.getDistance()<(DISTANCE - blockMoveCorrection)/8.0f*6.0f)
							{
								linetracer.linetrace(P_GAIN, I_GAIN, D_GAIN, TARGET_BRIGHTNESS, 60);
							}
						}
					};
	...
	private void createMissionScenario()
	{
